...
ShadingResult Raytracer::shade_separated(const HitRecord& hit, const Direction& view_dir) {
  ...
  Color total_diffuse(0, 0, 0);
  Color total_specular(0, 0, 0);
  ...
  for (const auto& light : scene.lights) {
    ...
    if (n_dot_l > 0.0) {
      ...
      if (n_dot_h > 0.0) {
        double spec = std::pow(n_dot_h, hit.material.shininess);
        Color specular = hit.material.specular_color * spec;
        total_specular += Color(specular.x * L_in.x, specular.y * L_in.y, specular.z * L_in.z);
      }
    }
  }

  return {ambient + total_diffuse, total_specular};
}

Color Raytracer::shade(const HitRecord& hit, const Direction& view_dir) {
  ShadingResult result = shade_separated(hit, view_dir);
...
Color Raytracer::trace(const Ray& ray, int depth) {
  ...
  if (intersect_bvh(ray, bvh_root.get(), scene, hit, 1e-5, t_max)) {
    ...
    // Get separated diffuse and specular components
    ShadingResult shading = is_pure_glass ? ShadingResult{Color(0,0,0), Color(0,0,0)}
                                          : shade_separated(hit, view_dir);
    Color color = shading.diffuse + shading.specular;
  ...
  for (int y = 0; y < height; y++) {
    ...
    for (int x = 0; x < width; x++) {
      ...
      for (int s = 0; s < samples_per_pixel; s++) {
        ...
        // If motion blur is disabled, use fixed time = 0.5 (middle of frame)
        double time = g_config.enable_motion_blur ? random_double() : 0.5;
